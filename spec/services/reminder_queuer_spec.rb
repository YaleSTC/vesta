...
RSpec.describe ReminderQueuer do
  ...
  context 'locked intent' do
    let(:draw) do
      instance_spy('draw', intent_locked?: true, intent_selection?: true)
    end
    let(:type) { 'intent' }
  ...
  context 'success' do
    let(:draw) do
      instance_spy('draw', update!: true, intent_locked?: false,
                           intent_selection?: true)
    end
