...
RSpec.describe DrawGroupFormationStarter do
  describe '#start' do
    it 'checks to make sure that the draw is in intent-selection' do
      draw = instance_spy('draw', intent_selection?: false)
      result = described_class.start(draw: draw)
      expect(result[:msg][:error])
        .to include('Draw must be in intent-selection.')
    end
    ...
    it 'sends invitations to the students in the draw' do
      ...
      described_class.new(draw: draw, mailer: mailer).start
      expect(mailer).to have_received(:group_formation).twice
    end
  ...
  def mock_draw_group_formation_starter(param_hash)
    instance_spy('draw_group_formation_starter').tap do |draw_formation|
      allow(described_class).to receive(:new).with(param_hash)
                                             .and_return(draw_formation)
    end
  end
