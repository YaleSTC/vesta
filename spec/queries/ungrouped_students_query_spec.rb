...
RSpec.describe UngroupedStudentsQuery do
  ...
  it 'returns only reps and students' do
    students = %i(student rep).map { |r| create(:user, role: r) }
    _admin = create(:admin)
    result = described_class.call
  ...
  it 'restricts the results to the passed query' do
    student1, student2 = create_pair(:student)
  ...
  def create_student_with_group
    ...
    draw.suites << suite
    create(:student, draw: draw).tap do |s|
      create(:group, draw: draw, size: suite.size, leader: s)
    end
  end
