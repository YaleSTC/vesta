...
RSpec.describe UsersByIntentQuery do
  ...
  it 'returns a hash mapping intents to students for the passed relation' do
    users = create_pair(:student_in_draw, draw: draw)
  ...
  it 'returns a hash mapping intents to users' do
    user1 = create(:student_in_draw, intent: 'on_campus', draw: draw)
    user2 = create(:student_in_draw, intent: 'off_campus', draw: draw)
    result = described_class.call
    expect(result).to eq('on_campus' => [user1], 'off_campus' => [user2])
  end

  it 'orders by intent' do
    user1 = create(:student_in_draw, intent: 'off_campus', draw: draw)
    user2 = create(:student_in_draw, intent: 'on_campus', draw: draw)
    result = described_class.call
    expect(result).to eq('on_campus' => [user2], 'off_campus' => [user1])
  end

  it 'orders by last name' do
    user1 = create(:student_in_draw, last_name: 'test2', draw: draw)
    user2 = create(:student_in_draw, last_name: 'test1', draw: draw)
    result = described_class.call
