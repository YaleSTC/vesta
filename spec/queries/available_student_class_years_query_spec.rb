...
RSpec.describe AvailableStudentClassYearsQuery do
  describe '#call' do
    it 'finds all the class years of available students' do
      ...
      create(:student_in_draw, class_year: 2017)
      create(:drawless_group).leader.update(class_year: 2018)
      expect(described_class.call).to eq([2016])
    end
    it 'ignores duplicates' do
      create_pair(:student, class_year: 2016)
      expect(described_class.call).to eq([2016])
    end
    it 'takes a relation in the constructor to limit the query' do
      create(:student, class_year: 2016)
      create(:student, class_year: 2017)
