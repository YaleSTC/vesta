...
Thanks so much for helping with the development of Vesta! This guide
...
please e-mail `stc-developers-rails{at}elilists.yale{dot}edu` or mention
@YaleSTC to contact the core team.

## Getting Started
There are many of ways you can contribute to the Vesta project, from
...
familiarize yourself with the application and the overall codebase. If
...
## Reporting an Issue
If you've been using Vesta and [notice a bug](#bug-reports),
or want to [suggest a new feature](#feature-requests), please open a
[GitHub issue](https://github.com/YaleSTC/vesta/issues/new).
Before doing so, however, please **search open / closed issues** to see if
someone else has already opened a similar issue.

### Bug Reports
A bug is a *verifiable problem* that is caused by the Vesta code. Good
...
* Vesta Version: [vX.X.X OR master (a1b2c3)]
...
form of user-, developer- , and sysadmin-facing documentation. Our developer-
and sysadmin-facing documentation is located in code comments, in our
[README](https://github.com/YaleSTC/vesta/blob/master/README.md) and in our
[GitHub wiki](https://github.com/YaleSTC/vesta/wiki).

We are in the process of setting up user-facing documentation using Github Pages.
...
changes to the wiki please [open up an issue](https://github.com/YaleSTC/vesta/issues/new)
and let us know what you think should be changed.

## Contributing Code
Helping to improve the Vesta codebase is **great**. In general, we only
...
[milestones](https://github.com/YaleSTC/vesta/milestones) for upcoming
versions; we'd prefer if you select open issues from those over issues in the
[Non-blocking Issues](https://github.com/YaleSTC/vesta/milestones/Non-blocking%20Issues).
Otherwise, feel free to select any open issues.

The following is an overview of how to work on Vesta:

### Fork Vesta / Create a Branch
Each developer working on Vesta should fork the core Vesta
repository and clone it as a local repository. Remember to add the core
repository as a remote:

```
git remote add upstream git@github.com:YaleSTC/Vesta.git
```
...
Vesta. You should run the rake task `rake check_style` which applies
rubocop to ensure that all of your changes match the project conventions.
We occasionally allow exceptions from code complexity violations; you should
make any such request in the GitHub pull request comments.
...
* The third line should reference the issue this PR resolves, either
  `issue #1234` if you're just mentioning the issue or `Resolves #1234` if
...
1. If you go to your fork's page on GitHub, there should be a prompt to submit
   a PR
   * If not go [here](https://github.com/YaleSTC/vesta/compare), select
     `YaleSTC/vesta` as the base fork and the relevant base branch and
...
3. Running the tests and style checks (either manually or via
   [Travis](https://travis-ci.org/YaleSTC/vesta)).
...
## Development Setup
The following are the major dependencies of Vesta:
...
Please read through the [README](https://github.com/YaleSTC/vesta/blob/master/README.md)
for information about setting up your local installation of Vesta.
...
As contributors and maintainers of the Vesta project, we pledge to
...
contributing to the Vesta project to do the same.

If any member of the community violates this code of conduct, the maintainers
of the Vesta project may take action, removing issues, comments, and
PRs or blocking accounts as deemed appropriate.

If you are subject to or witness unacceptable behavior, or have any other
concerns, please email us at `stc-developers-rails{at}elilists.yale{dot}edu`.